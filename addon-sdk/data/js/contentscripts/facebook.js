...
var handleMessage = function (streamElement, message) {
	console.debug("handling Message " + JSON.stringify(message));
	self.port.on("request" + message.id, function (response) {
		console.debug("handling response " + JSON.stringify(response));
		if (typeof response === 'object') {
			...
			if (response.urls) {
				...
				console.log("Inserting text " + response.plaintext);
				var textnode = document.createElement("p");
				textnode.innerHTML = response.plaintext.split("\n").join("<br/>");
	...
	self.port.emit("request", message);
};
...
var refresh = function () {
	...
	for (var i = 0, len = stories.length; i < len; i++) {
		...
		if (mb && !mb.hasAttribute("bm-injected")) {
			it = mb.innerHTML;
			console.debug("FBCS found a story! " + mb.innerHTML.substr(0,20));
			if (it !== null && typeof it !== 'undefined') {
				...
				if (bmtag !== -1) {
					...
					console.debug("FBCS is handling a message " + mb.hasAttribute("bm-injected"));
					handleMessage(mb, bm_message);
				}
			}
		}
