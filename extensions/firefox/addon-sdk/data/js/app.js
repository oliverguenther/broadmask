...
// Request to addon
function pluginCall(request, callback) {
	self.port.once(request.caller, 
	function(response) {
		callback(response);
	});
	self.port.emit("pluginCall", request);
}

// Simplyfy port messages into one call
function portRequest(request, callback) {
	self.port.once(request.method, callback);
	self.port.emit(request.method, request);
}
