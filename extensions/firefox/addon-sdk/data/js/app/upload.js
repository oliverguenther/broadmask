...
function enableShareForm(enable) {
	if (enable === true) {
		...
		$("#files-picker").removeAttr("disabled");
		...
		$("#files-picker").attr("disabled", "disabled");
...
function updateInstances(msg) {
	console.log("received " + JSON.stringify(msg));
	var instances = msg.response;
	if (instances.error === true) {

		// check if profile couldn't be loaded
...
$('#share-submit-btn').click(function () {
	...
	$(".thumb").each(function() {
		...
		image.success = function (xhr,	url) {
			...
			if (url) {
				statusicon.src = self.data.url("img/ok.png");
				$(progress).after("<p><a href=\"" + url +"\">Link</a></p>");
			} else {
				statusicon.src = self.data.url("img/warning.png");
			}
	...
	message.imghandler = 'picasa';
	message.handler = 'facebook';
	message.groupid = groupid;

	if (message.plaintext || message.media) {
		self.port.emit("shareMedia", message);
	}
